...
for n,index in enumerate(indexes_array):
	...
	if len(time_list[n]) > len(sig_1[:,0]):
		time_list_i = (time_list[n] <= sig_1[-1,0]) & (time_list[n] >= sig_1[0,0])
		time_list_n = time_list[n][time_list_i]
	else:
		len_n = len(time_list[n])
		
		sig_1_i = (sig_1[:,0] <= time_list[n][len_n-1]) & (sig_1[:,0] >= time_list[n][0])
		sig_1 = sig_1[sig_1_i]
		time_list_n = time_list[n]
	sig_2_y_velocity = np.array(y_velocity_list[n][time_list_i]) #construct the second signal which is to be correlated against
	#interpolated_sig_1 = np.interp(time_list_n, sig_1[:,0], sig_1[:,1]) #interpolate sig_1 (aoa) at the time points of the y_velocity array
	print('sig_1',len(sig_1))
	print('sig_2',len(sig_2_y_velocity))
	dt = time_list_n[1] - time_list_n[0]
	
	time_cross_corr,cross_norm = analysis.get_pearson_corr(sig_1[:,1],sig_2_y_velocity,dt) #calculate the pearson correlation of sig_1 (aoa) and each o the y_velocity
...
plt.ylabel(r'$R(v\acute{,}\alpha_i)$', fontsize=fontsize_var, fontweight='bold')
plt.xlim([-70,70])
plt.ylim([-1.0,1.0])
plt.grid(alpha=0.2, color='black')
plt.tight_layout()
plt.savefig(project_folder + save_path + 'xcorr_aoa_shear.pdf', dpi=300)
plt.close()
