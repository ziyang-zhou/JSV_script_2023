...
lips_bl_2d = 0.002
lips_bl_3d = 0.0018
bl_thickness_list = [lips_bl_3d,lips_bl_3d,lips_bl_3d,ff_bl_2d,lips_bl_2d]
U=16.0
rho=1.251
#############################################################################################
#                                        Load result                                        #
...
for data_folder in data_folder_list:
	...
	for file_name in sorted_files: #temp-probe-1.csv,temp-probe-5.csv,temp-probe-9.csv,etc...
...
plt.rc('legend', fontsize=30, fancybox=True, framealpha=0.7, edgecolor='black', facecolor='white', borderaxespad=0.5, labelspacing=0.5)
plt.rc('font', family='serif', weight='normal')

plt.tick_params(axis='both', which='major', labelsize=30)
ax = plt.gca()
ax.yaxis.offsetText.set(size=30)
...
for i,n in enumerate(plot_list):
	...
	for j,data_name in enumerate(name_list):
		...
		if '2D' in data_name:
			print(data_name,'identified as 2D')
			tstart_cfd = 0.5
			tend_cfd = 0.6
		else:
			tstart_cfd = eval(settings.at["tstart_cfd", settings.columns[0]])
		...
		plt.plot(f_cfd*bl_thickness_list[j]/U,10*np.log10(Pxx_cfd/(rho**2*bl_thickness_list[j]*U**3)),label = data_name, color = profile_color[j],linestyle=linestyle_list_b[j],linewidth=linewidth_list[j])

		print('t0',cfd_array_list[j][0,0],'t end',cfd_array_list[j][-1,0])


	plt.xlabel('Frequency [Hz]', fontsize=fontsize_var, fontweight='bold')
	plt.ylabel('SPL [dB/Hz]', fontsize=fontsize_var, fontweight='bold')
	ax=plt.gca()
	ax.set_xscale('log')
	plt.xlim((0.01, 10))
	plt.ylim((-120,0))
	...
	plt.savefig(project_folder + 'wall_psd/wall_psd_probe_{}_non_dim'.format(n))
	plt.close()
