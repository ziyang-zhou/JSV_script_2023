...
cfd_array_list = []
#Read CFD result
data_folder_list = ['cfd_refined_tripped','DNS_SLRT_damp_corrected/','DNS_SLRT_damp_corrected2/']
...
for i,n in enumerate(plot_list):
	...
	for j,data_name in enumerate(['SLRT','SLRT (damping corrected)','SLRT (damping corrected 2)']):
		print('plotting {}'.format(data_name))
		cfd_array_list = np.array(cfd_array_list)
		...
		#plt.plot(time[keep_time],mic_cfd[keep_time]-np.average(mic_cfd[keep_time]),label = data_name, color = profile_color[j],linestyle=linestyle_list_b[j],linewidth=linewidth_list[j])
		print('frequency resolution',f_cfd[1]-f_cfd[0])
		print('t0',cfd_array_list[j][keep_time][0,0],'t end',cfd_array_list[j][keep_time][-1,0])
	...
	plt.savefig(project_folder + 'wall_psd/DNS_SLRT_damp_corrected2/temp_wall_psd_probe_{}.jpg'.format(n))
	#plt.savefig(project_folder + 'wall_psd/DNS_SLRT_damp_corrected2/temp_wall_p_hist_probe_{}.pdf'.format(n))
	plt.close()
