...
#                                        Calculate profile                      #
#############################################################################################
profile_color = ['red','black','orange']
...
for probe in probe_number_list:
	...
	BL_damp_corrected2 = np.genfromtxt(project_folder + 'boundary_layer_profile/' + "BL_DNS_SLRT_damping_corrected2/probe_{}_vel.csv".format(probe_number), delimiter=',')
	...
	BL_damp_corrected[:, 0] /= chord
	BL_damp_corrected[:, 1] /= U_ref_list[2]
	BL_damp_corrected2[:, 0] /= chord
	BL_damp_corrected2[:, 1] /= U_ref_list[2]
	...
	plt.plot(BL_data_tripped[:, 1], BL_data_tripped[:, 0], label=name_list[2],color = profile_color[0],linestyle=linestyle_list_b[2],linewidth=linewidth_list[2])
	...
	plt.plot(BL_damp_corrected[:, 1], BL_damp_corrected[:, 0], label='SLRT Damping corrected',color = profile_color[1],linestyle=linestyle_list_b[3],linewidth=linewidth_list[3])
	plt.plot(BL_damp_corrected2[:, 1], BL_damp_corrected2[:, 0], label='SLRT Damping corrected 2',color = profile_color[2],linestyle=linestyle_list_b[3],linewidth=linewidth_list[3])
	plt.xlim([0, 2.2])
	plt.ylim([0, 0.08])
	...
	plt.savefig(project_folder + 'temp_probe_plot_{}.jpg'.format(probe_number), dpi=300)
